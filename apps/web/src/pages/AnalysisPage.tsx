...
import type { FieldTemplate, PromptTemplate, ApiError, Model } from '../types';
...
function inferModelProvider(model: Model): Model['provider'] {
...
function ModelProviderIcon({ provider }: { provider?: Model['provider'] }) {
  if (!provider) return null;

  return (
    <svg
      aria-hidden="true"
      viewBox={provider === 'claude' ? '0 0 256 257' : '0 0 256 260'}
      width={16}
      height={16}
      style={{ flex: '0 0 auto', color: provider === 'claude' ? '#d97757' : 'currentColor' }}
    >
      <path
        fill={provider === 'claude' ? '#d97757' : 'currentColor'}
        d={provider === 'claude' ? CLAUDE_ICON_PATH : OPENAI_ICON_PATH}
      />
    </svg>
  );
}
...
function ModelOptionLabel({ model }: { model: Model }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
      <ModelProviderIcon provider={inferModelProvider(model)} />

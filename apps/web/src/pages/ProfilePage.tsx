...
export default function ProfilePage() {
  ...
  const changePwMutation = useMutation({
    ...
    onError: (e: ApiError) => message.error(e.response?.data?.message || t('profile.changePasswordFailed')),
  });
    ...
    <Space direction="vertical" size={16} style={{ width: '100%', maxWidth: 600 }}>
      ...
      <Card>
        <Descriptions column={1}>
          ...
          <Descriptions.Item label={t('profile.role')}><Tag color={user?.role === 'admin' ? 'gold' : 'blue'}>{user?.role}</Tag></Descriptions.Item>
          <Descriptions.Item label={t('profile.authProvider')}><Tag>{user?.authProvider}</Tag></Descriptions.Item>
        </Descriptions>
        <Divider />
        <Button danger loading={logoutMutation.isPending} onClick={() => logoutMutation.mutate()}>{t('profile.logout')}</Button>
      </Card>
      ...
      {user?.authProvider === 'local' && (
        <Card title={t('profile.changePassword')}>
          <Form layout="vertical" onFinish={(v) => changePwMutation.mutate(v)}>
            ...
            <Button type="primary" htmlType="submit" loading={changePwMutation.isPending}>{t('profile.changePassword')}</Button>
          </Form>
        </Card>
      )}
    </Space>
